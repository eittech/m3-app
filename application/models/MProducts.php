...
class MProducts extends CI_Model {
    ...
    public function obtenerCombinacionesByCategory($id_category) {
		...
		$query = $this->db->where('p_l.id_lang', 1);
        $query = $this->db->group_by(array('p_l.name', 'a_l.id_attribute'));
        $query = $this->db->order_by('p.id_product, c.position, c_l.id_category, c_p.position, a.position');
        $query = $this->db->get();
    ...
    // Método público, para actualizar el precio de un producto
    public function update_price_shop($datos) {

        $result = $this->db->where('id_product', $datos['id_product']);
		$result = $this->db->update('product_shop', $datos);
		return $result;
